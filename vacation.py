...
def main():
    ...
    countries = {country["code"]: country["display"] for country in config["countries"]}

    holidays = {
        country: grab_holiday(this_year, country) for country in countries.keys()
    }
    ...
    with pandas.ExcelWriter(config["output"], engine="xlsxwriter") as writer:
        for month, month_data in data.items():
            ...
            to_write["Weekend"] = [
                weekend if "weekend" in month_data[date] else free for date in dates
            ]
            to_write.update(
